...
function PendingList() {
  ...
  const tasks = taskManageStore(state => state.pendingList);
  const pendingList = taskManageStore(state => state.pendingTaskList);
  const updateTask = taskManageStore(state => state.updateTask);
  const [updateStatus, setUpdateStatus] = useState(false)

  useEffect(() => {
    const run = async () => {
      await pendingList(user.id, token)
    }
    run()
  }, [updateStatus]);
  ...
  if (tasks.length > 0) {
    return (
      <>
        <div>
          <h1 className='text-2xl font-bold mt-5 mb-5'>Pending List</h1>
                      ...
                      {user.role == "LEADER" &&
                        <div className="dropdown dropdown-center dropdown-end">
                          ...
                          <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm abs ">
                            <li className='p-3 cursor-pointer hover:bg-base-300' onClick={() => hdlupdateTask(task.id, "APPROVEPENDING")}>Approve Pending</li>
                            <li className='p-3 cursor-pointer hover:bg-base-300' onClick={() => hdlupdateTask(task.id, "REJECT")}>Reject Pending</li>
                          </ul>
    ...
    return (
      <div className='flex justify-center items-center h-screen'>
        <div className='p-5 shadow-2xl rounded-xl'>
          <h1 className='text-2xl text-green-400 font-bold'>You have no request pending Task right now</h1>
        </div>
      </div>
    )
  }
